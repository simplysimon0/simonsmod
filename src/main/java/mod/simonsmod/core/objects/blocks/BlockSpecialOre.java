...
public class BlockSpecialOre extends Block implements IHasModel, IMetaName {
	public static final PropertyEnum<EnumMetal> VARIANT = PropertyEnum.<EnumMetal>create("variant",
			EnumMetal.class);

	private String name;

	public BlockSpecialOre(String name) {
		super(Material.ROCK);
	...
	public void registerModels() {
		for (int i = 0; i < EnumMetal.values().length; i++) {
			SimonsMod.proxy.registerVariantRenderer(Item.getItemFromBlock(this), i, name +"_"+ EnumMetal.values()[i].getName(), "inventory");
		}
	}
}