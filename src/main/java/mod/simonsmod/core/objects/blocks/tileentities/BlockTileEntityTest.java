...
public class BlockTileEntityTest extends Block implements ITileEntityProvider {
	...
	public Item getItemDropped(IBlockState state, Random rand, int fortune) {
		...
		return Item.getItemFromBlock(BlockInit.BLOCK_ALARM);
	}

	@Override
	public ItemStack getItem(World worldIn, BlockPos pos, IBlockState state) {
		
		return new ItemStack(BlockInit.BLOCK_ALARM);
	}
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn,
		...
		if (!worldIn.isRemote) {
			playerIn.openGui(SimonsMod.instance, GuiHandler.TEST, worldIn, pos.getX(), pos.getY(),
					pos.getZ());
		}
		return true;
	}
	...
	public static void setState(boolean active, World worldIn, BlockPos pos) 
	{
		...
		if(active) worldIn.setBlockState(pos, BlockInit.BLOCK_ALARM.getDefaultState().withProperty(FACING, state.getValue(FACING)).withProperty(ACTIVE, true), 3);
		else worldIn.setBlockState(pos, BlockInit.BLOCK_ALARM.getDefaultState().withProperty(FACING, state.getValue(FACING)).withProperty(ACTIVE, false), 3);
