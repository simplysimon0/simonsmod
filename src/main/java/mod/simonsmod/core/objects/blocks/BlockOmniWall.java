...
public class BlockOmniWall extends Block implements IHasModel{
	...
	public BlockOmniWall(String name, Material material, float hardness, float resistance) {
		super(material);
	...
	public IBlockState getActualState(IBlockState state, IBlockAccess worldIn, BlockPos pos) {
		...
		boolean flag4 = canBeConnectedToAngle(worldIn, pos, "northeast");
		boolean flag5 = canBeConnectedToAngle(worldIn, pos, "northwest");
		boolean flag6 = canBeConnectedToAngle(worldIn, pos, "southeast");
		boolean flag7 = canBeConnectedToAngle(worldIn, pos, "southwest");
		boolean flag9 = worldIn.getBlockState(pos.up()).equals(this.getDefaultState().withProperty(UP, true));
		boolean flag8 = flag && !flag1 && flag2 && !flag3 || !flag && flag1 && !flag2 && flag3;
		return state.withProperty(UP, Boolean.valueOf(!flag8 || flag9))
				...
				.withProperty(NORTHEAST, Boolean.valueOf(flag4))
				.withProperty(NORTHWEST, Boolean.valueOf(flag5))
				.withProperty(SOUTHEAST, Boolean.valueOf(flag6))
				.withProperty(SOUTHWEST, Boolean.valueOf(flag7));
	}

	protected BlockStateContainer createBlockState() {
		return new BlockStateContainer(this, new IProperty[] { UP, NORTH, EAST, WEST, SOUTH, NORTHEAST, SOUTHEAST, NORTHWEST, SOUTHWEST  });
	}
	...
	public boolean canBeConnectedTo(IBlockAccess world, BlockPos pos, EnumFacing facing) {
		Block connector = world.getBlockState(pos.add(1, 0, 1)).getBlock();
		Block connector1 = world.getBlockState(pos.add(-1, 0, 1)).getBlock();
		Block connector2 = world.getBlockState(pos.add(1, 0, -1)).getBlock();
		Block connector3 = world.getBlockState(pos.add(-1, 0, -1)).getBlock();
		Block connector4 = world.getBlockState(pos.offset(facing)).getBlock();
		return connector instanceof BlockOmniWall 
				|| connector1 instanceof BlockOmniWall
				|| connector2 instanceof BlockOmniWall
				|| connector3 instanceof BlockOmniWall
				|| connector4 instanceof BlockOmniWall;
	}
	

	public boolean canBeConnectedToAngle(IBlockAccess world, BlockPos pos, String angle) {
		Block connector = new Block(blockMaterial);
		if(angle.equals("northeast"))
		{
			if( world.getBlockState(pos.add(1,0,0)).getBlock()!=(this)&&world.getBlockState(pos.add(0,0,1)).getBlock()!=(this))
			{
				connector = world.getBlockState(pos.add(1, 0, 1)).getBlock();
			}
		}
		if(angle.equals("northwest"))
		{
			if( world.getBlockState(pos.add(-1,0,0)).getBlock()!=(this)&&world.getBlockState(pos.add(0,0,1)).getBlock()!=(this))
			{
				connector = world.getBlockState(pos.add(-1, 0, 1)).getBlock();
