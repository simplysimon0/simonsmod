...
public class TileEntityHopperOmniOld extends TileEntityLockableLoot implements IHopper, ITickable
{
    ...
    {
        fixer.registerWalker(FixTypes.BLOCK_ENTITY, new ItemStackDataLists(TileEntityHopperOmniOld.class, new String[] {"Items"}));
    }
     ...
     */
    public boolean isInventoryFull(IInventory inventoryIn, EnumFacing side)
    {
    ...
    {
    	EnumFacing enumfacing = BlockOmniHopperOld.getFacing(this.getBlockMetadata());
    	IInventory iinventory = getSourceInventoryUp(hopper);
    ...
    private static ItemStack insertStack(IInventory source, IInventory destination, ItemStack stack, int index, EnumFacing direction)
    {
        ...
        if (canInsertItemInSlot(destination, stack, index, direction))
        {
            ...
            if (flag)
            {
                if (flag1 && destination instanceof TileEntityHopperOmniOld)
                {
                    TileEntityHopperOmniOld tileentityhopper1 = (TileEntityHopperOmniOld)destination;

                    if (!tileentityhopper1.mayTransfer())
                    {
                        int k = 0;

                        if (source != null && source instanceof TileEntityHopperOmniOld)
                        {
                            TileEntityHopperOmniOld tileentityhopper = (TileEntityHopperOmniOld)source;
