...
public class TileEntityHopperDuct extends TileEntityLockableLoot implements IHopper, ITickable
{
    ...
    {
        fixer.registerWalker(FixTypes.BLOCK_ENTITY, new ItemStackDataLists(TileEntityHopperDuct.class, new String[] {"Items"}));
    }

    public int[] getSlotsForFace(EnumFacing side) {
		return this.slots_HopperDuct;
	}
    ...
    private static ItemStack insertStack(IInventory source, IInventory destination, ItemStack stack, int index, EnumFacing direction)
    {
        ...
        if (canInsertItemInSlot(destination, stack, index, direction))
        {
            ...
            if (flag)
            {
                if (flag1 && destination instanceof TileEntityHopperDuct)
                {
                    TileEntityHopperDuct tileentityhopper1 = (TileEntityHopperDuct)destination;

                    if (!tileentityhopper1.mayTransfer())
                    {
                        int k = 0;

                        if (source != null && source instanceof TileEntityHopperDuct)
                        {
                            TileEntityHopperDuct tileentityhopper = (TileEntityHopperDuct)source;
