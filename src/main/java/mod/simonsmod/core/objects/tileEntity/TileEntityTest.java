...
import mod.simonsmod.core.handlers.recipe.CrusherRecipes;
...
public class TileEntityTest extends TileEntity implements IInventory, ITickable
{
	...
	public EnergyStorageTest storage;
	
	public TileEntityTest() {
		this.storage = new EnergyStorageTest(5000, 10);
	}
	@Override
	...
	{
		return this.hasCustomName() ? this.customName : "container.tile_entity_test";
	}
	...
	public void readFromNBT(NBTTagCompound compound)
	{
		...
		this.storage.energy = compound.getInteger("BurnTime");
	...
	public NBTTagCompound writeToNBT(NBTTagCompound compound) 
	{
		...
		compound.setInteger("BurnTime", (short)this.storage.energy);
	...
	public void update() 
	{
		...
		if(this.isActive()) --this.storage.energy;
		...
		if(!this.world.isRemote) 
		{
			...
			if(this.isActive() || !stack.isEmpty() && !((((ItemStack)this.inventory.get(0)).isEmpty()) || ((ItemStack)this.inventory.get(1)).isEmpty())) 
			{
				...
				{
					//Consumes the Fuel
					this.storage.energy = getItemBurnTime(stack);
					this.currentBurnTime = this.storage.energy;
			...
			{
				flag1 = true;
				BlockTileEntityTest.setState(this.isActive(), this.world, this.pos);
			}
		} 
	...
	private boolean canSmelt() 
	{
		...
		{
			ItemStack result = CrusherRecipes.getInstance().getSinteringResult((ItemStack)this.inventory.get(0), (ItemStack)this.inventory.get(1));	
	...
	public void smeltItem() 
	{
		if(this.canSmelt()) 
		{
			...
			ItemStack result = CrusherRecipes.getInstance().getSinteringResult(input1, input2);
			ItemStack output = (ItemStack)this.inventory.get(3);
	...
	{
		return "simonsmod:tileentiytest";
	}
