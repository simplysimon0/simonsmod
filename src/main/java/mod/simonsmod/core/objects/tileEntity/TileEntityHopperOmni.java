...
import mod.simonsmod.core.objects.blocks.tileentities.BlockOmniHopper;
...
public class TileEntityHopperOmni extends TileEntityLockableLoot implements IHopper, ITickable
{
    private NonNullList<ItemStack> inventory = NonNullList.<ItemStack>withSize(5, ItemStack.EMPTY);
    private int transferCooldown = -1;
    private long tickedGameTime;
    public int regularFacing = 0;
    public int extendedFacing = 0;
    private NBTTagCompound hopperTagCompound;
    public static void registerFixesHopper(DataFixer fixer)
    {
        fixer.registerWalker(FixTypes.BLOCK_ENTITY, new ItemStackDataLists(TileEntityHopperOmni.class, new String[] {"Items"}));
    }
    ...
    public void readFromNBT(NBTTagCompound compound)
    {
        ...
        this.extendedFacing 	= compound.getInteger("ExtendedFacing");
        this.regularFacing 		= compound.getInteger("RegularFacing");
    ...
    public NBTTagCompound writeToNBT(NBTTagCompound compound)
    {
        ...
        compound.setInteger("ExtendedFacing", this.extendedFacing);
        compound.setInteger("RegularFacing", this.regularFacing);
    ...
    {
        if (this.hopperTagCompound != null && this.hopperTagCompound.hasKey(key, 10))
        {
            return this.hopperTagCompound.getCompoundTag(key);
        }
        else
        {
    ...
    {
        if (this.hopperTagCompound == null)
        {
            this.setTagCompound(new NBTTagCompound());
        }

        this.hopperTagCompound.setTag(key, value);
    }
    
    public void setTagCompound(@Nullable NBTTagCompound nbt)
    {
        this.hopperTagCompound = nbt;
    }
    ...
    public void setExtendedFacing(EnumFacing facing)
    {
    ...
    public EnumFacing getExtendedFacing()
    {
    	if(extendedFacing == 0)
    		return EnumFacing.DOWN;
    	else if(extendedFacing == 1)
    		return EnumFacing.UP;
    	else if(extendedFacing == 2)
    		return EnumFacing.NORTH;
    	else if(extendedFacing == 3)
    		return EnumFacing.SOUTH;
    	else if(extendedFacing == 4)
    		return EnumFacing.EAST;
    	else if(extendedFacing == 5)
    		return EnumFacing.WEST;
    	return EnumFacing.DOWN;
    }
    
    public void setRegularFacing(EnumFacing facing)
    {
    	if(facing == EnumFacing.DOWN)
    		regularFacing = 0;
    	else if(facing == EnumFacing.UP)
    ...
    public EnumFacing getRegularFacing()
    {
    	if(regularFacing == 0)
    		return EnumFacing.DOWN;
    	else if(regularFacing == 1)
    		return EnumFacing.UP;
    	else if(regularFacing == 2)
    		return EnumFacing.NORTH;
    	else if(regularFacing == 3)
    		return EnumFacing.SOUTH;
    	else if(regularFacing == 4)
    		return EnumFacing.EAST;
    	else if(regularFacing == 5)
    		return EnumFacing.WEST;
    	return EnumFacing.DOWN;
    }
    ...
    {
        return this.hasCustomName() ? this.customName : "container.hopper_omni";
    }
    ...
    private boolean transferItemsOut()
    {
        ...
        {
        	EnumFacing enumfacing = BlockOmniHopper.getFacing(this.getBlockMetadata(), false).getOpposite();
    ...
    {
    	EnumFacing enumfacing = BlockOmniHopper.getFacing(this.getBlockMetadata(), true).getOpposite();
    	IInventory iinventory;
        if(enumfacing == EnumFacing.UP)
        	iinventory = getSourceInventoryUp(hopper);
        else if(enumfacing == EnumFacing.DOWN)
        	iinventory = getSourceInventoryDown(hopper);
        else if(enumfacing == EnumFacing.NORTH)
        	iinventory = getSourceInventorySouth(hopper);
        else if(enumfacing == EnumFacing.EAST)
        	iinventory = getSourceInventoryWest(hopper);
        else if(enumfacing == EnumFacing.SOUTH)
        	iinventory = getSourceInventoryNorth(hopper);
        else
        	iinventory = getSourceInventoryEast(hopper);
    ...
    private static ItemStack insertStack(IInventory source, IInventory destination, ItemStack stack, int index, EnumFacing direction)
    {
        ...
        if (canInsertItemInSlot(destination, stack, index, direction))
        {
            ...
            if (flag)
            {
                if (flag1 && destination instanceof TileEntityHopperOmni)
                {
                    TileEntityHopperOmni tileentityhopper1 = (TileEntityHopperOmni)destination;

                    if (!tileentityhopper1.mayTransfer())
                    {
                        int k = 0;

                        if (source != null && source instanceof TileEntityHopperOmni)
                        {
                            TileEntityHopperOmni tileentityhopper = (TileEntityHopperOmni)source;
    ...
    {
        EnumFacing enumfacing = BlockHopperDuct.getFacing(this.getBlockMetadata());
     ...
     */
    public static IInventory getSourceInventoryUp(IHopper hopper)
    {
        return getInventoryAtPosition(hopper.getWorld(), hopper.getXPos(), hopper.getYPos() + 1.0D, hopper.getZPos());
    }
    
    public static IInventory getSourceInventoryDown(IHopper hopper)
    {
        return getInventoryAtPosition(hopper.getWorld(), hopper.getXPos(), hopper.getYPos() - 1.0D, hopper.getZPos());
    }
    
    public static IInventory getSourceInventoryWest(IHopper hopper)
    {
        return getInventoryAtPosition(hopper.getWorld(), hopper.getXPos() + 1.0D, hopper.getYPos(), hopper.getZPos());
    }
    
    public static IInventory getSourceInventoryEast(IHopper hopper)
    {
        return getInventoryAtPosition(hopper.getWorld(), hopper.getXPos() - 1.0D, hopper.getYPos(), hopper.getZPos());
    }
    
    public static IInventory getSourceInventoryNorth(IHopper hopper)
    {
        return getInventoryAtPosition(hopper.getWorld(), hopper.getXPos(), hopper.getYPos(), hopper.getZPos() + 1.0D);
    }
    
    public static IInventory getSourceInventorySouth(IHopper hopper)
    {
        return getInventoryAtPosition(hopper.getWorld(), hopper.getXPos(), hopper.getYPos(), hopper.getZPos() - 1.0D);
    }
