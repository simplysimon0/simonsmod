...
import mod.simonsmod.core.world.LootTables;
...
public class LabGneissWorldGen extends WorldGenerator {
	Random r2 = new Random();
	public static final ResourceLocation LAB = new ResourceLocation(Reference.MODID + ":loot_tables/chests/lab.json");
	...
	public boolean generate(World world, Random rand, BlockPos position) {
		...
		Template template = templatemanager.getTemplate(minecraftserver,
				new ResourceLocation(Reference.MODID + ":lab_gneiss"));
		...
		if (SimonWorldGenerator.canSpawnHere(template, worldserver, position)) {
			...
			for (Entry<BlockPos, String> entry : map.entrySet()) {
				if ("chest".equals(entry.getValue())) {
					...
					if (tileentity instanceof TileEntityChest) {
						((TileEntityChest) tileentity).setLootTable(LootTables.LAB, rand.nextLong());
					}
				}
			}
