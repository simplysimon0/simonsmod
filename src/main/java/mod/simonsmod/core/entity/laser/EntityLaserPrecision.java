...
public class EntityLaserPrecision extends Entity implements IProjectile {
	...
	public EntityLaserPrecision(World worldIn) {
		super(worldIn);
		this.xTile = -1;
		this.yTile = -1;
		this.zTile = -1;
	...
	public EntityLaserPrecision(World worldIn, double x, double y, double z) {
		this(worldIn);
		this.setPosition(x, y, z);
	}

	public EntityLaserPrecision(World worldIn, EntityLivingBase shooter) {
	...
	protected void onHit(RayTraceResult raytraceResultIn) {
		...
		if (entity != null) {
			...
			EntityItem andesite = new EntityItem(this.world, (double) this.xTile, (double) this.yTile,
					(double) this.zTile, new ItemStack(BlockInit.BLOCK_ANDESITE, 1, 0));
			EntityItem diorite = new EntityItem(this.world, (double) this.xTile, (double) this.yTile,
					(double) this.zTile, new ItemStack(BlockInit.BLOCK_DIORITE, 1, 0));
			EntityItem granite = new EntityItem(this.world, (double) this.xTile, (double) this.yTile,
					(double) this.zTile, new ItemStack(BlockInit.BLOCK_GRANITE, 1, 0));
			this.setIsCritical(false);
			if (iblockstate == Blocks.BEDROCK.getDefaultState()) {
				this.setDead();
			} else {
				if (MainConfig.getCobbleVanillaOres) {
					if (iblockstate == Blocks.STONE.getDefaultState().withProperty(BlockStone.VARIANT,
							EnumType.ANDESITE)) {
						this.world.spawnEntity(andesite);
					} else if (iblockstate == Blocks.STONE.getDefaultState().withProperty(BlockStone.VARIANT,
							EnumType.DIORITE)) {
						this.world.spawnEntity(diorite);
					} else if (iblockstate == Blocks.STONE.getDefaultState().withProperty(BlockStone.VARIANT,
							EnumType.GRANITE)) {
						this.world.spawnEntity(granite);
					} else {
						this.inTile.dropBlockAsItem(this.world, blockpos, iblockstate, 0);
					}
				} else {
					this.inTile.dropBlockAsItem(this.world, blockpos, iblockstate, 0);
				}

				this.setBlock(this.world, blockpos, Blocks.AIR.getDefaultState());
			}
	...
	static {
		CRITICAL = EntityDataManager.createKey(EntityLaserPrecision.class, DataSerializers.BYTE);
	}
}