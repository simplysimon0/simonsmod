...
public class EntityLaserEntity extends Entity implements IProjectile {
	...
	public EntityLaserEntity(World worldIn) {
		super(worldIn);
		this.xTile = -1;
		this.yTile = -1;
		this.zTile = -1;
	...
	public EntityLaserEntity(World worldIn, double x, double y, double z) {
		this(worldIn);
		this.setPosition(x, y, z);
	}

	public EntityLaserEntity(World worldIn, EntityLivingBase shooter) {
			...
			if (!this.world.isRemote) {
				if (raytraceResultIn.entityHit != null) {
					raytraceResultIn.entityHit
							.attackEntityFrom(DamageSource.causeThrownDamage(this, this.shootingEntity), 8.0F);
					entity.setFire(5);
				}
	...
	static {
		CRITICAL = EntityDataManager.createKey(EntityLaserEntity.class, DataSerializers.BYTE);
	}
}